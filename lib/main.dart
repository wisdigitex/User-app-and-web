...
import 'package:sixam_mart/util/app_utils.dart';
import 'package:sixam_mart/util/images.dart';
  ...
  if (GetPlatform.isWeb) {
    ...
    await Firebase.initializeApp(
      options: const FirebaseOptions(
        apiKey: "AIzaSyBj6pdirJRGK-bkFTJ14nxySjM_ITqjj-w",
        appId: "1:987650730174:android:c5529d545abb76716996ef",
        messagingSenderId: "987650730174",
        projectId: "eniza-hypermarket",
      ),
    );
...
class _MyAppState extends State<MyApp> {
  ...
  void _route() async {
    if (GetPlatform.isWeb) {
      ...
      if (!AuthHelper.isLoggedIn() && !AuthHelper
          .isGuestLoggedIn() /*&& !ResponsiveHelper.isDesktop(Get.context!)*/) {
        await Get.find<AuthController>().guestLogin();
      }

      if ((AuthHelper.isLoggedIn() || AuthHelper.isGuestLoggedIn()) && Get
    ...
    Get.find<SplashController>().getConfigData(
        loadLandingData: GetPlatform.isWeb).then((bool isSuccess) async {
      if (isSuccess) {
          ...
          return (GetPlatform.isWeb && splashController.configModel == null)
            ...
            builder: (BuildContext context, widget) {
              return MediaQuery(data: MediaQuery.of(context).copyWith(
                  textScaler: const TextScaler.linear(1)), child: Material(
                child: Stack(children: [
                  ...
                  GetBuilder<SplashController>(builder: (splashController) {
                    if (!splashController.savedCookiesData &&
                        !splashController.getAcceptCookiesStatus(splashController.configModel != null ? splashController.configModel!.cookiesText! : '')) {
                      return ResponsiveHelper.isWeb() ? const Align(
                          alignment: Alignment.bottomCenter,
                          child: CookiesView()) : const SizedBox();
                    } else {
                          ...
                          child: FloatingActionButton(
                            onPressed: () async {
                              ...
                              if (ResponsiveHelper.isMobile(context)) {
                                debugPrint("Screen size is less");
                                AppUtils.whatsapp(
                                    Uri.parse(AppConstants.whatsAppIOSUri));
                              } else if (ResponsiveHelper.isWeb()) {
                                AppUtils.whatsapp(
                                    Uri.parse(AppConstants.whatsAppWebUri));
                              }
                            },
                            backgroundColor: Colors.transparent,
                            child: Image.asset(Images.whatsappIcon),
                          ),
                        ),
                      );
                    }
