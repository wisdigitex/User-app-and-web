...
class AuthService implements AuthServiceInterface{
  ...
  Future<ResponseModel> login({String? phone, String? password, required bool isCustomerVerificationOn}) async {
    ...
    if (response.statusCode == 200) {
      print("APIs Login Response : ${response.body}");
    ...
    @override
    Future<ResponseModel> emailLogin({String? email, String? password}) async {
        Response response = await authRepositoryInterface.emailLogin(email: email, password: password);
        ResponseModel responseModel;
        if (response.statusCode == 200) {
        print("APIs Login Response : ${response.body}");
        if(isCustomerVerificationOn && response.body['is_phone_verified'] == 0) {
  ...
  Future<void>  saveUserEmailAndPassword(String email, String password)  async{
    await authRepositoryInterface.saveUserEmailAndPassword(email, password);
  }
